...
2. `npm install`
3. Run `npm run watch` in one terminal and `npm run dev-server` in another
4. Upload `manifest.xml` to Word on the web via **Home → Add-ins → Upload My Add-in**
5. **Important for Word Online:** Enable Track Changes manually in the **Review** tab before running redaction
6. Click **Run Redaction** in the task pane

**Note:** This solution was tested on Word on the web. If using Word Desktop, Track Changes may be enabled automatically by the add-in.
