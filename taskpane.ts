...
type MatchKind = "email" | "phone" | "ssn";
...
function getButton() {
  return document.getElementById("redact-btn") as HTMLButtonElement | null;
}
...
function findSensitivePatterns(text: string): SensitiveMatch[] {
  const results: SensitiveMatch[] = [];

  // Helper to add unique matches
  function addMatches(regex: RegExp, kind: MatchKind) {
...
function getStats(matches: SensitiveMatch[]) {
...
function buildMessage(stats: ReturnType<typeof getStats>, trackingMsg: string) {
  const parts: string[] = [];
...
async function setupTrackChanges(context: Word.RequestContext): Promise<string> {
  let note = "";
...
async function insertHeader(context: Word.RequestContext) {
  try {
...
async function performRedaction(context: Word.RequestContext, matches: SensitiveMatch[]): Promise<number> {
  let count = 0;
